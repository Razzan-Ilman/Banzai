...
@section('content')
    <div class="card" style="max-width: 600px;">
        <div class="card-header">Tambah Divisi Baru</div>
        <div class="card-body">
            <form action="{{ route('admin.divisions.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="tagline" name="tagline" class="form-input" value="{{ old('tagline') }}" placeholder="Contoh: Melestarikan Bahasa Jepang">
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" id="icon" name="icon" class="form-input" value="{{ old('icon', '📚') }}" placeholder="📚">
                    </div>
                    
                    <div class="form-group">
                        <label for="color" class="form-label">Warna</label>
                        <input type="color" id="color" name="color" value="{{ old('color', '#0891B2') }}" style="width: 100%; height: 40px; border: none; border-radius: 0.375rem;">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <select id="character" name="character" class="form-select">
                            ...
                            <option value="Scholar" {{ old('character') == 'Scholar' ? 'selected' : '' }}>Scholar</option>
                            <option value="Artist" {{ old('character') == 'Artist' ? 'selected' : '' }}>Artist</option>
                            <option value="Connector" {{ old('character') == 'Connector' ? 'selected' : '' }}>Connector</option>
                        </select>
                    </div>
